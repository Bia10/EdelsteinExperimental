...
public sealed class IndexingStatus
{
    ...
    public IEnumerable<IndexProgress> GetProgress() =>
        _progress.Values.OrderBy(static p => p.State);
    ...
    public void IncrementTrieRetrieveException(string commandName) =>
        _trieTelemetry.AddOrUpdate(
            ...
            static (_, existing) =>
                existing with
                {
                    RetrieveExceptions = existing.RetrieveExceptions + 1,
                }
        );
    ...
    public string GetProgressMessage()
    {
        ...
        Array.Sort(
            snapshot,
            static (a, b) => GetDisplayPriority(a.State).CompareTo(GetDisplayPriority(b.State))
        );
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static int GetDisplayPriority(IndexState state) =>
        state switch
        {
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static void AppendGroupSuffix(
        StringBuilder sb,
        IndexState state,
