...
public class PluginCommand : AbstractCommand
{
    ...
    {
        var id = 0;
        var plugins = (await _pluginManager.RetrieveAll()).ToImmutableDictionary(h => id++, h => h);
        var pluginMenu = plugins.ToImmutableDictionary(
            kv => kv.Key,
            kv => kv.Value.Manifest?.Name ?? kv.Value.ID
        );
        var pluginSelect = await user.Prompt(
            target => target.AskMenu("Here are the currently loaded plugins", pluginMenu),
            -1
        );
