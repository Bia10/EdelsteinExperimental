...
public class LoginDiagnostics
{
    ...
    {
        if (
            FlowMilestones.TryGetValue(from, out var fromTime)
    ...
    /// </summary>
    public static bool HasBothTimelines =>
        _savedManualTimeline != null && _savedAutoTimeline != null;

    /// <summary>
    /// Directory for saving diagnostic reports.
    /// </summary>
    public static string ReportsDirectory { get; set; } =
        Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "plugins", "Rue", "Reports");
    ...
    public static string GenerateComparison()
    {
        ...
        sb.AppendLine(
            "║              MANUAL vs AUTO LOGIN COMPARISON                                         ║"
        );
        sb.AppendLine(
            "╚══════════════════════════════════════════════════════════════════════════════════════╝"
        );
        ...
        if (_savedManualTimeline == null)
        {
            ...
            sb.AppendLine(
                "  Run with WatchMode='passive', perform manual login, then the timeline is auto-saved."
            );
            sb.AppendLine();
        }
        ...
        // Header info
        sb.AppendLine(
            $"Manual Login: {_savedManualTimeline.CaptureTime:HH:mm:ss} ({_savedManualTimeline.TotalDuration.TotalSeconds:F1}s, {_savedManualTimeline.Events.Length} events)"
        );
        sb.AppendLine(
            $"Auto Login:   {_savedAutoTimeline.CaptureTime:HH:mm:ss} ({_savedAutoTimeline.TotalDuration.TotalSeconds:F1}s, {_savedAutoTimeline.Events.Length} events)"
        );
        sb.AppendLine();
        ...
        var manualEvents = _savedManualTimeline
            .Events.Select(e =>
                (
                    Time: (
                        e.Timestamp
                        - _savedManualTimeline.CaptureTime
                        + _savedManualTimeline.TotalDuration
                    ).TotalMilliseconds,
                    Event: e,
                    Source: "M"
                )
            ...
            .ToList();
        var autoEvents = _savedAutoTimeline
            .Events.Select(e =>
                (
                    Time: (
                        e.Timestamp
                        - _savedAutoTimeline.CaptureTime
                        + _savedAutoTimeline.TotalDuration
                    ).TotalMilliseconds,
                    Event: e,
                    Source: "A"
                )
        ...
        {
            var manualEvent = FindEventByCategory(
                [.. manualEvents.Select(x => x.Event)],
                category,
                _savedManualTimeline.CaptureTime
            );
            var autoEvent = FindEventByCategory(
                [.. autoEvents.Select(x => x.Event)],
                category,
                _savedAutoTimeline.CaptureTime
            );
            ...
            var manualStr =
                manualEvent != null
                    ? $"+{manualEvent.Value.relMs, 6:F0}ms {manualEvent.Value.desc}"
                    : "---";
            var autoStr =
                autoEvent != null
                    ? $"+{autoEvent.Value.relMs, 6:F0}ms {autoEvent.Value.desc}"
                    : "---";

            // Truncate to fit columns
            if (manualStr.Length > 34)
                manualStr = manualStr[..31] + "...";
            if (autoStr.Length > 44)
                autoStr = autoStr[..41] + "...";
        ...
        if (ms != null && auto != null)
        {
            ...
            CompareField(
                sb,
                "CUIChannelSelect.Exists",
                ms.ChannelSelectExists,
                auto.ChannelSelectExists
            );
            CompareField(sb, "CUIChannelSelect.Selected", ms.SelectedChannel, auto.SelectedChannel);
            CompareField(
                sb,
                "ConnectionDlg.Exists",
                ms.ConnectionDlgExists,
                auto.ConnectionDlgExists
            );
        }
        ...
        {
            sb.AppendLine(
                "  ✓ No significant differences detected between manual and auto login final states."
            );
        }
        else
        {
    ...
    private static (double relMs, string desc)? FindEventByCategory(
    ...
    )
    {
        ...
        foreach (var evt in events)
        {
            ...
            if (
                category == "CheckUserLimitResult"
                && evt.Description.Contains("CheckUserLimitResult")
            )
            {
                found = evt;
                break;
            ...
            if (
                category == "CUIWorldSelect"
                && evt.Details?.TryGetValue("changes", out var c1) == true
                && c1 is List<string> l1
                && l1.Any(s => s.Contains("CUIWorldSelect"))
            )
            {
                found = evt;
                break;
            ...
            if (
                category == "CUIChannelSelect"
                && evt.Details?.TryGetValue("changes", out var c2) == true
                && c2 is List<string> l2
                && l2.Any(s => s.Contains("CUIChannelSelect"))
            )
            {
                found = evt;
                break;
            ...
            if (
                category == "WorldId"
                && evt.Details?.TryGetValue("changes", out var c3) == true
                && c3 is List<string> l3
                && l3.Any(s => s.Contains("WorldId:"))
            )
            {
                found = evt;
                break;
            ...
            if (
                category == "ChannelId"
                && evt.Details?.TryGetValue("changes", out var c4) == true
                && c4 is List<string> l4
                && l4.Any(s => s.Contains("ChannelId:"))
            )
            {
                found = evt;
                break;
            ...
            if (
                category == "RequestSent"
                && evt.Details?.TryGetValue("changes", out var c5) == true
                && c5 is List<string> l5
                && l5.Any(s => s.Contains("RequestSent:"))
            )
            {
                found = evt;
                break;
        ...
        if (found == null)
            return null;
        return (
            (found.Timestamp - baseTime).TotalMilliseconds,
            found.Description.Length > 30 ? found.Description[..30] : found.Description
        );
    }
    ...
    private static List<string> FindDifferences(MemorySnapshot? manual, MemorySnapshot? auto)
    {
        ...
        if (manual.RequestSent != auto.RequestSent)
            diffs.Add(
                $"RequestSent: manual={manual.RequestSent}, auto={auto.RequestSent} - CRITICAL: must be true!"
            );
        if (manual.WorldIdx != auto.WorldIdx)
            diffs.Add(
                $"CUIWorldSelect.WorldIdx: manual={manual.WorldIdx}, auto={auto.WorldIdx} - affects CUIChannelSelect creation"
            );
        if (manual.ChannelSelectExists != auto.ChannelSelectExists)
            diffs.Add(
                $"CUIChannelSelect exists: manual={manual.ChannelSelectExists}, auto={auto.ChannelSelectExists}"
            );
        if (manual.ConnectionDlgExists != auto.ConnectionDlgExists)
            diffs.Add(
                $"ConnectionDlg exists: manual={manual.ConnectionDlgExists}, auto={auto.ConnectionDlgExists} - created by SendLoginPacket"
            );
    ...
    public void LogMemoryChange(MemorySnapshot? oldSnapshot, MemorySnapshot newSnapshot)
    {
        ...
        else
        {
            ...
            if (oldSnapshot.CharacterCount != newSnapshot.CharacterCount)
                changes.Add(
                    $"CharacterCount: {oldSnapshot.CharacterCount} -> {newSnapshot.CharacterCount}"
                );
            ...
            if (oldSnapshot.ChannelNameArrayPtr != newSnapshot.ChannelNameArrayPtr)
                changes.Add(
                    $"ChannelNamePtr: {FormatPtr(oldSnapshot.ChannelNameArrayPtr)} -> {FormatPtr(newSnapshot.ChannelNameArrayPtr)}"
                );
            if (oldSnapshot.AdultChannelArrayPtr != newSnapshot.AdultChannelArrayPtr)
                changes.Add(
                    $"AdultChannelPtr: {FormatPtr(oldSnapshot.AdultChannelArrayPtr)} -> {FormatPtr(newSnapshot.AdultChannelArrayPtr)}"
                );
            ...
            if (oldSnapshot.StepChanging != newSnapshot.StepChanging)
                changes.Add(
                    $"StepChanging: {oldSnapshot.StepChanging} -> {newSnapshot.StepChanging}"
                );
            if (oldSnapshot.CharSelected != newSnapshot.CharSelected)
                changes.Add(
                    $"CharSelected: {oldSnapshot.CharSelected} -> {newSnapshot.CharSelected}"
                );

            // CUIChannelSelect changes
            if (oldSnapshot.ChannelSelectExists != newSnapshot.ChannelSelectExists)
                changes.Add(
                    $"CUIChannelSelect: {(oldSnapshot.ChannelSelectExists ? "exists" : "null")} -> {(newSnapshot.ChannelSelectExists ? "exists" : "null")}"
                );
            if (oldSnapshot.SelectedChannel != newSnapshot.SelectedChannel)
                changes.Add(
                    $"SelectedChannel: {oldSnapshot.SelectedChannel} -> {newSnapshot.SelectedChannel}"
                );
            if (oldSnapshot.WorldItemPtr != newSnapshot.WorldItemPtr)
                changes.Add(
                    $"WorldItemPtr: {FormatPtr(oldSnapshot.WorldItemPtr)} -> {FormatPtr(newSnapshot.WorldItemPtr)}"
                );
            if (oldSnapshot.ConnectionDlgExists != newSnapshot.ConnectionDlgExists)
                changes.Add(
                    $"ConnectionDlg: {(oldSnapshot.ConnectionDlgExists ? "exists" : "null")} -> {(newSnapshot.ConnectionDlgExists ? "exists" : "null")}"
                );

            // CUIWorldSelect changes
            if (oldSnapshot.WorldSelectExists != newSnapshot.WorldSelectExists)
                changes.Add(
                    $"CUIWorldSelect: {(oldSnapshot.WorldSelectExists ? "exists" : "null")} -> {(newSnapshot.WorldSelectExists ? "exists" : "null")}"
                );
    ...
    {
        var evt = new DiagnosticEvent(DateTime.UtcNow, source, "Info", message, null);
        _events.Enqueue(evt);
    }
    ...
    public string GenerateReport()
    {
        ...
        if (lastSnapshot != null)
        {
            ...
            {
                sb.AppendLine(
                    "(Note: This is the state AFTER crash cleanup - may show null values"
                );
                sb.AppendLine(
                    " as the client destroys UI singletons when returning to title screen)"
                );
                sb.AppendLine();
            }
    ...
    private static DiagnosticEvent? FindMatchingEvent(DiagnosticEvent[] events, string stepId)
    {
        return stepId switch
        {
            ...
            "CUIWorldSelectCreated" => events.FirstOrDefault(e =>
                ...
                && c is List<string> list
                && list.Any(s => s.Contains("CUIWorldSelect") && s.Contains("exists"))
            ),
            "CheckUserLimit" => events.FirstOrDefault(e =>
                e.Description.Contains("CheckUserLimit")
                && e.Description.Contains("C->S")
            ...
            "CUIChannelSelectCreated" => events.FirstOrDefault(e =>
                ...
                && e.Details?.TryGetValue("changes", out var c) == true
                && c is List<string> list
                && list.Any(s => s.Contains("CUIChannelSelect") && s.Contains("exists"))
            ),
            "SelectWorld" => events.FirstOrDefault(e =>
                e.Description.Contains("SelectWorld")
                && e.Description.Contains("C->S")
                && !e.Description.Contains("Result")
            ),
            ...
            "CharCountSet" => events.FirstOrDefault(e =>
                ...
                && c is List<string> list
                && list.Any(s =>
                    s.Contains("CharacterCount") && !s.Contains("-> 0") && !s.Contains("->0")
                )
            ),
            _ => null,
        };
    ...
    private static string AnalyzeFlow(DiagnosticEvent[] events, MemorySnapshot? finalState)
    {
        ...
        if (finalState != null)
        {
            ...
            if (finalState.RequestSent == null)
                issues.Add(
                    "WARNING: CLogin->m_bRequestSent is null - CLogin may not be accessible"
                );
            else if (finalState.RequestSent == false)
                issues.Add(
                    "WARNING: CLogin->m_bRequestSent is false - should be true before SelectWorldResult"
                );
            else
            ...
            if (
                finalState.LoginStep != null
                && (finalState.LoginStep < 0 || finalState.LoginStep > 10)
            )
                issues.Add(
                    $"CRITICAL: CLogin->m_nLoginStep={finalState.LoginStep} looks like garbage - wrong CLogin address?"
                );
            ...
            if (!finalState.ChannelSelectExists)
                issues.Add(
                    "WARNING: CUIChannelSelect does not exist - CheckUserLimitResult not processed?"
                );
            else
            ...
            if (!finalState.WorldSelectExists)
                issues.Add(
                    "WARNING: CUIWorldSelect does not exist - WorldInformation not processed?"
                );
            else
            ...
            if (finalState.StepChanging != null && finalState.StepChanging > 0)
                observations.Add(
                    $"m_tStepChanging={finalState.StepChanging} (GetTickCount - client transitioning between steps)"
                );
        ...
        var worldSelectCreated = memoryEvents.FirstOrDefault(e =>
            ...
            && changes is List<string> list
            && list.Any(c => c.Contains("CUIWorldSelect") && c.Contains("exists"))
        );

        var channelSelectCreated = memoryEvents.FirstOrDefault(e =>
            e.Details?.TryGetValue("changes", out var changes) == true
            && changes is List<string> list
            && list.Any(c => c.Contains("CUIChannelSelect") && c.Contains("exists"))
        );

        var crashEvent = events.FirstOrDefault(e =>
            e.Description.Contains("crash") || e.Description.Contains("ClientDumpLog")
        );
        ...
        {
            var timeBetween = (
                channelSelectCreated.Timestamp - worldSelectCreated.Timestamp
            ).TotalMilliseconds;
        ...
        if (crashEvent != null)
        {
            ...
            if (crashEvent.Details?.TryGetValue("type", out var crashType) == true)
            {
                ...
                if (typeStr.Contains("0xB") || typeStr.Contains("11"))
                    issues.Add(
                        "CRASH in SelectWorldResult (0x0B) handler - check OnSelectWorldResult flow"
                    );
                else if (typeStr.Contains("0x3") || typeStr.Contains('3'))
                    issues.Add(
                        "CRASH in CheckUserLimitResult (0x03) handler - CUIWorldSelect may not exist"
                    );
            }
        }
