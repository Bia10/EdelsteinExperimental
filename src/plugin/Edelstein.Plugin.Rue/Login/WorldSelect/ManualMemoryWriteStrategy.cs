...
public sealed class ManualMemoryWriteStrategy : IWorldSelectStrategy
{
    public async Task Execute(WorldSelectContext context)
    {
        ...
        context.Logger?.LogInformation(
            "[Rue-AutoLogin] Writing CWvsContext world={WorldId}, ch={ChannelId} (manual-write)",
            context.WorldId,
            context.ChannelId
        );
        ...
        context.Logger?.LogDebug(
            "[Rue-AutoLogin] Pre-write: Step={Step}, StepChg={StepChg}, ReqSent={ReqSent}",
            preLoginStep,
            preStepChanging,
            preRequestSent
        );
        ...
        context.Diagnostics?.LogMemoryWrite(
            "CWvsContext->m_nChannelID",
            preChannelId,
            context.ChannelId
        );
        ...
        if (!writer.SetWorldAndChannel(context.WorldId, context.ChannelId))
        {
            context.Logger?.LogError(
                "[Rue-AutoLogin] Failed to write world/channel to CWvsContext"
            );
            context.Diagnostics?.LogError(
                "AutoLogin",
                "Failed to write world/channel to CWvsContext"
            );
            return;
        }
        ...
        if (!writer.SetQuestManWorldId(context.WorldId))
            context.Logger?.LogWarning(
                "[Rue-AutoLogin] Failed to mirror WorldID to CQuestMan — ChangeStep may crash"
            );
        else
            context.Logger?.LogInformation(
                "[Rue-AutoLogin] Mirrored CQuestMan.m_nWorldID = {WorldId}",
                context.WorldId
            );
        ...
        if (context.Monitor != null)
        {
            ...
            if (!stableOk)
                context.Logger?.LogWarning(
                    "[Rue-AutoLogin] Step transition timeout - proceeding anyway"
                );
        }
        else
        {
            ...
            {
                context.Logger?.LogInformation(
                    "[Rue-AutoLogin] Waiting {Delay}ms for step transition (delay-based)",
                    delay
                );
        ...
        context.Logger?.LogInformation(
            "[Rue-AutoLogin] Step transition complete (+{Elapsed})",
            LoginMilestonesTracker.FormatElapsed(elapsed)
        );
        ...
        if (context.Config.ClientMemory?.FixupChannelSelectState ?? true)
        {
            if (writer.EnsureChannelSelectState(context.WorldId, context.ChannelId))
            {
                ...
                context.Logger?.LogInformation(
                    "[Rue-AutoLogin] Fixup: CUIChannelSelect selected={Selected}, worldItemPtr=0x{Ptr:X8}",
                    selected,
                    worldItemPtr ?? 0
                );

                context.Diagnostics?.LogMemoryWrite(
                    "CUIChannelSelect->m_nSelect",
                    preSelectedChannel,
                    selected
                );
                context.Diagnostics?.LogMemoryWrite(
                    "CUIChannelSelect->m_pWorldItem",
                    preWorldItemPtr,
                    worldItemPtr
                );
            }
            ...
            {
                context.Logger?.LogWarning(
                    "[Rue-AutoLogin] Fixup failed to set CUIChannelSelect world item / selection"
                );
            }
        }
        ...
        if (writer.EnsureContextChannelArraysFromWorldItem(context.WorldId))
        {
            ...
            context.Logger?.LogInformation(
                "[Rue-AutoLogin] Fixup: CWvsContext channel arrays set (names=0x{Names:X8}, adult=0x{Adult:X8})",
                channelNamePtr ?? 0,
                adultChannelPtr ?? 0
            );

            context.Diagnostics?.LogMemoryWrite(
                "CWvsContext->m_aChannelName.a",
                preChannelNamePtr,
                channelNamePtr
            );
            context.Diagnostics?.LogMemoryWrite(
                "CWvsContext->m_aAdultChannel.a",
                preAdultChannelPtr,
                adultChannelPtr
            );
        }
        ...
        {
            context.Logger?.LogWarning(
                "[Rue-AutoLogin] Fixup failed to set CWvsContext channel arrays"
            );
        }
        ...
        {
            context.Logger?.LogWarning(
                "[Rue-AutoLogin] CLogin not found - cannot set m_bRequestSent"
            );
        }
        ...
        context.Diagnostics?.LogPacketSent(
            "SelectWorldResult",
            0x0B,
        ...
        await context.Context.Pipelines.UserOnPacketSelectWorld.Process(
            new UserOnPacketSelectWorld(context.User, context.WorldId, context.ChannelId)
        );
