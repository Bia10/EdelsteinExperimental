...
public sealed class DirectFunctionCallStrategy : IWorldSelectStrategy
{
    public async Task Execute(WorldSelectContext context)
    {
        ...
        context.Logger?.LogInformation(
            "[Rue-AutoLogin] DFC: calling SendLoginPacket({WorldId}, {ChannelId})",
            context.WorldId,
            context.ChannelId
        );
        ...
        if (!writer.FindCLogin())
        {
            context.Logger?.LogError(
                "[Rue-AutoLogin] DFC: cannot find CLogin - ensure CUIChannelSelect exists"
            );
            context.Diagnostics?.LogError(
                "AutoLogin-DFC",
                "CLogin not found for direct function call"
            );
            return;
        }

        context.Logger?.LogDebug(
            "[Rue-AutoLogin] DFC pre-call: World={World}, Ch={Ch}, Step={Step}, ReqSent={ReqSent}",
            writer.ReadWorldId(),
        ...
        context.Logger?.LogInformation(
            "[Rue-AutoLogin] DFC: SendLoginPacket -> success (+{Elapsed})",
        ...
        if (context.Monitor != null)
        {
            ...
            if (!stepOk)
                context.Logger?.LogWarning(
                    "[Rue-AutoLogin] DFC: SelectCharacter timeout - proceeding"
                );
        }
        else
        {
        ...
        context.Logger?.LogDebug(
            "[Rue-AutoLogin] DFC post-call: Step={Step}, ReqSent={ReqSent}, CharSel={CharSel}",
            writer.ReadLoginStep(),
            writer.ReadRequestSent(),
        ...
        context.Logger?.LogInformation(
            "[Rue-AutoLogin] DFC: complete ({TotalMs}ms)",
            Environment.TickCount64 - t0
        );
    }
}
