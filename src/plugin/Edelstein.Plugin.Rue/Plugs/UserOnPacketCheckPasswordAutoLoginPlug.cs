...
public class UserOnPacketCheckPasswordAutoLoginPlug(
    ILogger? logger,
...
) : IPipelinePlug<UserOnPacketCheckPassword>
{
    ...
    public async Task Handle(IPipelineContext ctx, UserOnPacketCheckPassword message)
    {
        ...
        var loginResult = await _context.Services.Auth.Login(
            new AuthRequest(message.Username, message.Password)
        );

        if (loginResult.Result == AuthResult.FailedInvalidUsername)
        {
            await _context.Services.Auth.Register(
                new AuthRequest(message.Username, message.Password)
            );
            _logger?.LogInformation(
                "[Rue-AutoLogin] Auto-registered new account: {Username}",
                message.Username
            );
        }
    }
