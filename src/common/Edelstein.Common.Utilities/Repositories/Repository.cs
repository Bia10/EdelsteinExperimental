    ...
    where TEntry : IIdentifiable<TKey>
{
    ...
    public virtual Task Delete(TKey key) => Task.FromResult(_dictionary.Remove(key));

    public virtual Task Delete(TEntry entry) => Task.FromResult(_dictionary.Remove(entry.ID));

    public virtual Task<ICollection<TEntry>> RetrieveAll() => Task.FromResult(_dictionary.Values);
}
