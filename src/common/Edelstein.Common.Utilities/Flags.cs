...
public readonly struct Flags : IPacketWritable
{
    ...
    public Flags(int size) => _bits = new BitArray(size);

    public Flags(BitArray bits) => _bits = bits;

    public void SetFlag(int index, bool value = true) => _bits.Set(index, value);

    public bool HasFlag(int index) => _bits.Get(index);
