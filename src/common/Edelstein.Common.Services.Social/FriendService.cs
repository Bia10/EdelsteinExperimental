...
public class FriendService : IFriendService
{
    ...
    public async Task<FriendLoadResponse> Load(FriendLoadRequest request)
    {
        try
        {
            ...
            var friends =
                db.Friends.Where(f => f.CharacterID == request.CharacterID)
                    .ToDictionary(f => f.FriendID, f => (IFriend)f) as IDictionary<int, IFriend>;
    ...
    public async Task<FriendResponse> Invite(FriendInviteRequest request)
    {
        try
        {
            ...
            if (
                await db.FriendProfiles.AnyAsync(p =>
                    p.CharacterID == request.InviterID && p.IsMaster
                ) || await db.FriendProfiles.AnyAsync(p => p.CharacterID == target.ID && p.IsMaster)
            )
                return new FriendResponse(FriendResult.FailedMaster);

            var inviterFriendCount = await db.Friends.CountAsync(f =>
                f.CharacterID == request.InviterID
            );
            var targetFriendCount = await db.Friends.CountAsync(f => f.CharacterID == target.ID);

            if (
                await db.FriendProfiles.AnyAsync(p =>
                    p.CharacterID == request.InviterID && inviterFriendCount >= p.FriendMax
                )
            )
                return new FriendResponse(FriendResult.FailedMaxSlotMe);
            if (
                await db.FriendProfiles.AnyAsync(p =>
                    p.CharacterID == target.ID && targetFriendCount >= p.FriendMax
                )
            )
            ...
            var friends = db
                .Friends.Where(f => f.CharacterID == request.InviterID)
                .ToDictionary(f => f.FriendID, f => (IFriend)f);
            var friendList = new FriendList(friends);

            await _messaging.PublishAsync(
                new NotifyFriendUpdateList(request.InviterID, friendList)
            );
            await _messaging.PublishAsync(
                new NotifyFriendInvited(
                    request.InviterID,
                    request.InviterName,
    ...
    public async Task<FriendResponse> InviteAccept(FriendInviteAcceptRequest request)
    {
        ...
        {
            await using var db = await _dbFactory.CreateDbContextAsync();
            var record1 = await db.Friends.FirstOrDefaultAsync(f =>
                f.CharacterID == request.FriendID && f.FriendID == request.InviterID
            );
            var record2 = await db.Friends.FirstOrDefaultAsync(f =>
                f.CharacterID == request.InviterID && f.FriendID == request.FriendID
            );
            ...
            var friends1 = db
                .Friends.Where(f => f.CharacterID == request.FriendID)
                .ToDictionary(f => f.FriendID, f => (IFriend)f);
            var friends2 = db
                .Friends.Where(f => f.CharacterID == request.InviterID)
                .ToDictionary(f => f.FriendID, f => (IFriend)f);
            ...
            await _messaging.PublishAsync(
                new NotifyFriendUpdateList(request.FriendID, new FriendList(friends1))
            );
            await _messaging.PublishAsync(
                new NotifyFriendUpdateList(request.InviterID, new FriendList(friends2))
            );
            return new FriendResponse(FriendResult.Success);
        }
        catch (Exception)
    ...
    public async Task<FriendResponse> Delete(FriendDeleteRequest request)
    {
        try
        {
            await using var db = await _dbFactory.CreateDbContextAsync();
            var record1 = await db.Friends.FirstOrDefaultAsync(f =>
                f.CharacterID == request.CharacterID && f.FriendID == request.FriendID
            );
            var record2 = await db.Friends.FirstOrDefaultAsync(f =>
                f.CharacterID == request.FriendID && f.FriendID == request.CharacterID
            );
            ...
            var friends1 = db
                .Friends.Where(f => f.CharacterID == request.CharacterID)
                .ToDictionary(f => f.FriendID, f => (IFriend)f);
            var friends2 = db
                .Friends.Where(f => f.CharacterID == request.FriendID)
                .ToDictionary(f => f.FriendID, f => (IFriend)f);
            ...
            await _messaging.PublishAsync(
                new NotifyFriendUpdateList(request.CharacterID, new FriendList(friends1))
            );
            await _messaging.PublishAsync(
                new NotifyFriendUpdateList(request.FriendID, new FriendList(friends2))
            );
            return new FriendResponse(FriendResult.Success);
        }
        catch (Exception)
    ...
    public async Task<FriendResponse> UpdateProfile(FriendUpdateProfileRequest request)
    {
        try
        {
            ...
            if (profile)
                await db
                    .FriendProfiles.Where(f => f.CharacterID == request.CharacterID)
                    .ExecuteUpdateAsync(p =>
                        p.SetProperty(e => e.FriendMax, e => request.FriendMax)
                            .SetProperty(e => e.IsMaster, e => request.IsMaster)
                    );
            else
            {
                await db.FriendProfiles.AddAsync(
                    new FriendProfileEntity
                    {
    ...
    public async Task<FriendResponse> UpdateGroup(FriendUpdateGroupRequest request)
    {
        try
        {
            ...
            await db
                ...
                .ExecuteUpdateAsync(p =>
                    p.SetProperty(e => e.FriendGroup, e => request.FriendGroup)
                );
            var friends =
                db.Friends.Where(f => f.CharacterID == request.CharacterID)
                    .ToDictionary(f => f.FriendID, f => (IFriend)f) as IDictionary<int, IFriend>;
            await _messaging.PublishAsync(
                new NotifyFriendUpdateList(request.CharacterID, new FriendList(friends))
            );
            return new FriendResponse(FriendResult.Success);
        }
        catch (Exception)
    ...
    public async Task<FriendResponse> UpdateChannel(FriendUpdateChannelRequest request)
    {
        try
        {
            await using var db = await _dbFactory.CreateDbContextAsync();
            await db
                .Friends.Where(f => f.FriendID == request.CharacterID)
                .ExecuteUpdateAsync(p => p.SetProperty(e => e.ChannelID, e => request.ChannelID));
            await _messaging.PublishAsync(
                new NotifyFriendUpdateChannel(request.CharacterID, request.ChannelID)
            );
            return new FriendResponse(FriendResult.Success);
        }
        catch (Exception)
