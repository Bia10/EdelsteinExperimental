...
public class PartyService : IPartyService
{
    ...
    public async Task<PartyLoadResponse> Load(PartyLoadRequest request)
    {
        try
        {
            ...
            var partyMember = db
                .PartyMembers.Include(p => p.Party)
                    .ThenInclude(p => p.Members)
                .FirstOrDefault(p => p.CharacterID == request.CharacterID);
            return new PartyLoadResponse(
                PartyResult.Success,
                partyMember != null ? new PartyMembership(partyMember) : null
            );
        }
        catch (Exception)
        {
    ...
    public async Task<PartyResponse> Create(PartyCreateRequest request)
    {
        try
        {
            ...
            var party = new PartyEntity { BossCharacterID = request.CharacterID };
            ...
            await _messaging.PublishAsync(
                new NotifyPartyCreated(request.CharacterID, new PartyMembership(partyMember))
            );
    ...
    public async Task<PartyResponse> Disband(PartyDisbandRequest request)
    {
        try
        {
            ...
            if (
                !await db.Parties.AnyAsync(p =>
                    p.ID == request.PartyID && p.BossCharacterID == request.CharacterID
                )
            )
                return new PartyResponse(PartyResult.FailedNotBoss);
            await db.Parties.Where(p => p.ID == request.PartyID).ExecuteDeleteAsync();
            await _messaging.PublishAsync(
                new NotifyPartyDisbanded(request.CharacterID, request.PartyID)
            );
            return new PartyResponse(PartyResult.Success);
        }
        catch (Exception)
    ...
    public async Task<PartyResponse> Leave(PartyLeaveRequest request)
    {
        try
        {
            ...
            await db
                .PartyMembers.Where(p =>
                    p.PartyID == request.PartyID && p.CharacterID == request.CharacterID
                )
                .ExecuteDeleteAsync();
            await _messaging.PublishAsync(
                new NotifyPartyMemberWithdrawn(
                    request.PartyID,
                    request.CharacterID,
    ...
    public async Task<PartyResponse> Invite(PartyInviteRequest request)
    {
        try
        {
            ...
            if (
                !await db.Parties.AnyAsync(p =>
                    p.ID == request.PartyID && p.BossCharacterID == request.InviterID
                )
            )
                return new PartyResponse(PartyResult.FailedNotBoss);
            ...
            if (
                await db.PartyInvitations.AnyAsync(i =>
                    i.PartyID == request.PartyID && i.CharacterID == target.ID && i.DateExpire > now
                )
            )
                return new PartyResponse(PartyResult.FailedAlreadyInvited);

            await db
                .PartyInvitations.Where(i =>
                    i.PartyID == request.PartyID && i.CharacterID == target.ID
                )
                .ExecuteDeleteAsync();
            await db.PartyInvitations.AddAsync(
                new PartyInvitationEntity
                {
    ...
    public async Task<PartyResponse> InviteAccept(PartyInviteAcceptRequest request)
    {
        try
        {
            await using var db = await _dbFactory.CreateDbContextAsync();
            var now = DateTime.UtcNow;
            var invitation = await db.PartyInvitations.FirstOrDefaultAsync(i =>
                i.PartyID == request.PartyID && i.CharacterID == request.CharacterID
            );

            if (invitation == null || invitation.DateExpire < now)
                return new PartyResponse(PartyResult.FailedNotInvited);
            if (
                await db.PartyMembers.AnyAsync(m =>
                    m.PartyID == request.PartyID && m.CharacterID == request.CharacterID
                )
            )
                return new PartyResponse(PartyResult.FailedAlreadyInParty);
            ...
            partyMember = await db
                .PartyMembers.Include(m => m.Party)
                    .ThenInclude(p => p.Members)
                .FirstAsync(m =>
                    m.PartyID == request.PartyID && m.CharacterID == request.CharacterID
                );
    ...
    public async Task<PartyResponse> Kick(PartyKickRequest request)
    {
        try
        {
            ...
            if (
                !await db.Parties.AnyAsync(p =>
                    p.ID == request.PartyID && p.BossCharacterID == request.BossID
                )
            )
                return new PartyResponse(PartyResult.FailedNotBoss);
            var partyMember = db.PartyMembers.FirstOrDefault(p =>
                p.PartyID == request.PartyID && p.CharacterID == request.CharacterID
            );
    ...
    public async Task<PartyResponse> ChangeBoss(PartyChangeBossRequest request)
    {
        try
        {
            ...
            await db
                .Parties.Where(p => p.ID == request.PartyID)
                .ExecuteUpdateAsync(p =>
                    p.SetProperty(e => e.BossCharacterID, e => request.CharacterID)
                );
            await _messaging.PublishAsync(
                new NotifyPartyChangedBoss(
                    request.PartyID,
                    request.CharacterID,
    ...
    public async Task<PartyResponse> UpdateChannelOrField(PartyUpdateChannelOrFieldRequest request)
    {
        try
        {
            ...
            await db
                ...
                .ExecuteUpdateAsync(p =>
                    p.SetProperty(e => e.ChannelID, e => request.ChannelID)
                        .SetProperty(e => e.FieldID, e => request.FieldID)
                );
            await _messaging.PublishAsync(
                new NotifyPartyMemberUpdateChannelOrField(
                    request.PartyID,
                    request.CharacterID,
                    request.ChannelID,
                    request.FieldID
    ...
    public async Task<PartyResponse> UpdateLevelOrJob(PartyUpdateLevelOrJobRequest request)
    {
        try
        {
            ...
            await db
                ...
                .ExecuteUpdateAsync(p =>
                    p.SetProperty(e => e.Level, e => request.Level)
                        .SetProperty(e => e.Job, e => request.Job)
                );
            await _messaging.PublishAsync(
                new NotifyPartyMemberUpdateLevelOrJob(
                    request.PartyID,
                    request.CharacterID,
                    request.Level,
                    request.Job
