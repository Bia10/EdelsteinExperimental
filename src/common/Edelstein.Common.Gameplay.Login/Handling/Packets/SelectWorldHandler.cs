...
public class SelectWorldHandler
    : AbstractPipedPacketHandler<ILoginStageUser, UserOnPacketSelectWorld>
{
    public SelectWorldHandler(IPipeline<UserOnPacketSelectWorld> pipeline)
        : base(pipeline) { }
    ...
    public override UserOnPacketSelectWorld Serialize(ILoginStageUser user, IPacketReader reader) =>
        new(user, reader.Skip(1).ReadByte(), reader.ReadByte());
}
