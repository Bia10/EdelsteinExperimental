...
public class PluginManager<TContext>
    : Repository<string, IPluginHost<TContext>>,
        IPluginManager<TContext>
{
    ...
    public async Task LoadFromFile(string path, IPluginHostManifest? manifest)
    {
        ...
        {
            _logger.LogWarning(
                "Failed to load plugins from path {Path} as file does not exist",
                path
            );
            return;
        }
    ...
    public async Task LoadFromDirectory(string directory)
    {
        ...
        {
            _logger.LogWarning(
                "Failed to load plugins from path {Path} as directory does not exist",
                directory
            );
            return;
        }
