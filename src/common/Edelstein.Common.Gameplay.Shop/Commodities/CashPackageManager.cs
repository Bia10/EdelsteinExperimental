...
public class CashPackageManager : ICashPackageManager
{
    ...
    public CashPackageManager(
        ITemplateManager<ICashPackageTemplate> templates,
    ...
    public async Task<ICashPackage?> Retrieve(int key)
    {
        ...
        var modified = (await _modifiedManager.RetrieveAll()).FirstOrDefault(m => m.ItemID == key);

        if (template == null && modified?.PackageSN == null)
            return null;
