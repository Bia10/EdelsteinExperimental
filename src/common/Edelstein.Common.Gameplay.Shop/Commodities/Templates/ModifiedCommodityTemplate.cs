...
public record ModifiedCommodityTemplate : IModifiedCommodity, ITemplate
{
    public ModifiedCommodityTemplate(int sn, IDataNode node)
    {
        ...
        PackageSN =
            node.ResolvePath("PackageSN")?.Select(c => c.ResolveInt() ?? 0).ToFrozenSet() ?? null;

        if (ItemID != null)
            Flags |= CommodityFlags.ItemID;
        if (Count != null)
            Flags |= CommodityFlags.Count;
        if (Priority != null)
            Flags |= CommodityFlags.Priority;

        if (Price != null)
            Flags |= CommodityFlags.Price;
        if (Bonus != null)
            Flags |= CommodityFlags.Bonus;

        if (Period != null)
            Flags |= CommodityFlags.Period;
        if (ReqPOP != null)
            Flags |= CommodityFlags.ReqPOP;
        if (ReqLevel != null)
            Flags |= CommodityFlags.ReqLVL;

        if (MaplePoint != null)
            Flags |= CommodityFlags.MaplePoint;
        if (Meso != null)
            Flags |= CommodityFlags.Meso;

        if (ForPremiumUser != null)
            Flags |= CommodityFlags.ForPremiumUser;

        if (Gender != null)
            Flags |= CommodityFlags.CommodityGender;

        if (OnSale != null)
            Flags |= CommodityFlags.OnSale;
        if (Class != null)
            Flags |= CommodityFlags.Class;
        if (Limit != null)
            Flags |= CommodityFlags.Limit;

        if (PbCash != null)
            Flags |= CommodityFlags.PbCash;
        if (PbPoint != null)
            Flags |= CommodityFlags.PbPoint;
        if (PbGift != null)
            Flags |= CommodityFlags.PbGift;

        if (PackageSN != null)
            Flags |= CommodityFlags.PackageSN;
    }
