...
{
    public ShopStageUser(ISocket socket, ShopContext context)
    ...
    public override Task Migrate(string serverID, IPacket? packet = null) =>
        Context.Pipelines.UserMigrate.Process(
            new UserMigrate<IShopStageUser>(this, serverID, packet)
        );
    ...
    public override Task OnException(Exception exception) =>
        Context.Pipelines.UserOnException.Process(
            new UserOnException<IShopStageUser>(this, exception)
        );
