...
public class ShopOnPacketCashItemIncSlotCountRequestPlug
    : IPipelinePlug<ShopOnPacketCashItemIncSlotCountRequest>
{
    public async Task Handle(IPipelineContext ctx, ShopOnPacketCashItemIncSlotCountRequest message)
    {
        ...
        if (!message.User.CheckCash(message.Cash, 4000))
            return;
        if (inventory == null)
            return;
        if (inventory.SlotMax + 4 > 96)
            return;
