...
public class ShopOnPacketCashItemBuyPackageRequestPlug
    : IPipelinePlug<ShopOnPacketCashItemBuyPackageRequest>
{
    ...
    public ShopOnPacketCashItemBuyPackageRequestPlug(
    ...
    public async Task Handle(IPipelineContext ctx, ShopOnPacketCashItemBuyPackageRequest message)
    {
        ...
        if (message.User.AccountWorld?.Locker == null)
            return;
        if (
            message.User.AccountWorld.Locker.Items.Count
            >= message.User.AccountWorld.Locker.SlotMax - cashPackage.SN.Count + 1
        )
            return;
