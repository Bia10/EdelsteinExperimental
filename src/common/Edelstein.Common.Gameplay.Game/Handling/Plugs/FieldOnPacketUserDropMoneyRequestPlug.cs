...
public class FieldOnPacketUserDropMoneyRequestPlug
    : IPipelinePlug<FieldOnPacketUserDropMoneyRequest>
{
    public async Task Handle(IPipelineContext ctx, FieldOnPacketUserDropMoneyRequest message)
    {
        if (message.Money is < 10 or > 50000)
            return;
        if (message.User.Character.Money < message.Money)
            return;

        var foothold = message
            .User.Field?.Template.Footholds.FindBelow(
                new Point2D(message.User.Position.X, message.User.Position.Y - 100)
            )
            .FirstOrDefault();
        ...
        var drop = new FieldDropMoney(position, message.Money) { OwnType = DropOwnType.NoOwn };
