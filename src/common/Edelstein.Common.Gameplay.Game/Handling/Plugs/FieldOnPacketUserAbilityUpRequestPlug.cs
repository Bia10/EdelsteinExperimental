...
public class FieldOnPacketUserAbilityUpRequestPlug
    : IPipelinePlug<FieldOnPacketUserAbilityUpRequest>
{
    ...
    public Task Handle(IPipelineContext ctx, FieldOnPacketUserAbilityUpRequest message) =>
        _plug.Handle(
            ctx,
            new FieldOnPacketUserAbilityMassUpRequest(
                message.User,
                new Dictionary<ModifyStatType, int> { [message.Type] = 1 }
            )
        );
}
