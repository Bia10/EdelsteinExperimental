...
public class FieldOnPacketUserChangeSlotPositionRequestPlug
    : IPipelinePlug<FieldOnPacketUserChangeSlotPositionRequest>
{
    public async Task Handle(
        IPipelineContext ctx,
        FieldOnPacketUserChangeSlotPositionRequest message
    )
    {
        if (message.To == 0)
        {
            ...
            var drop = new FieldDropItem(position, item) { OwnType = DropOwnType.NoOwn };
