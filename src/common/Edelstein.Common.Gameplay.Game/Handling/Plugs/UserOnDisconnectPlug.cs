...
public class UserOnDisconnectPlug : AbstractUserOnDisconnectPlug<IGameStageUser>
{
    ...
    public override async Task Handle(
        ...
        UserOnDisconnect<IGameStageUser> message
    )
    {
        ...
        {
            message.User.Character.FieldID =
                message.User.Field.Template.ForcedReturn ?? message.User.Field.Template.ID;
            message.User.Character.FieldPortal = (byte)(
                message.User.Field.Template.ForcedReturn != null
                    ? 0
                    : message
                        .User.Field.Template.StartPoints.FindClosest(
                            message.User.FieldUser.Position
                        )
                        .FirstOrDefault()
                        ?.ID
                        ?? 0
