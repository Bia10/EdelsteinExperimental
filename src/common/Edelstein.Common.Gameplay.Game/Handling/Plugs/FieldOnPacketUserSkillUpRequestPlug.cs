...
public class FieldOnPacketUserSkillUpRequestPlug : IPipelinePlug<FieldOnPacketUserSkillUpRequest>
{
    ...
    public FieldOnPacketUserSkillUpRequestPlug(ITemplateManager<ISkillTemplate> templates) =>
    ...
    public async Task Handle(IPipelineContext ctx, FieldOnPacketUserSkillUpRequest message)
    {
        ...
        if (template.IsInvisible)
            return;
        if (
            template.ReqSkill.Any(kv =>
                (message.User.Character.Skills[kv.Key]?.Level ?? 0) < kv.Value
            )
        )
            return;
        ...
        if ((message.User.Character.Skills[template.ID]?.Level ?? 0) + increment > maxLevel)
            return;
        ...
        if (skillJobLevel == 0)
        {
            ...
            if (JobConstants.GetBeginnerJob(job) != skillJob)
                return;
        ...
        if (
            JobConstants.GetJobRace(job) != JobConstants.GetJobRace(skillJob)
        ...
        if (
            JobConstants.IsExtendSPJob(job)
            && message.User.Character.ExtendSP[skillJobLevel] < increment
        )
            return;
        if (!JobConstants.IsExtendSPJob(job) && message.User.Character.SP < increment)
            return;
