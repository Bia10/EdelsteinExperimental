...
public record QuestTemplateCheck : IQuestTemplateCheck
{
    public QuestTemplateCheck(IDataNode? node)
    {
        ...
        Jobs = node?.ResolvePath("job")?.Children.Select(c => c.ResolveInt() ?? -1).ToFrozenSet();
        SubJobFlags = node?.ResolveInt("subJobFlags");

        CheckItem = node
            ?.ResolvePath("item")
            ?.Children.Select(p => (IQuestTemplateCheckItem)new QuestTemplateCheckItem(p.Cache()))
            .ToFrozenSet();
        CheckMob = node
            ?.ResolvePath("mob")
            ?.Children.Select(p =>
                (IQuestTemplateCheckMob)
                    new QuestTemplateCheckMob(Convert.ToInt32(p.Name), p.Cache())
            )
