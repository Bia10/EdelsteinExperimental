...
public record QuestTemplateAct : IQuestTemplateAct
{
    public QuestTemplateAct(IDataNode? node)
    {
        ...
        Items = node
            ?.ResolvePath("item")
            ?.Children.Select(p =>
                (IQuestTemplateActItem)new QuestTemplateActItem(Convert.ToInt32(p.Name), p.Cache())
            )
            .ToFrozenSet();
        Skills = node
            ?.ResolvePath("skill")
            ?.Children.Select(p => (IQuestTemplateActSkill)new QuestTemplateActSkill(p.Cache()))
            .ToFrozenSet();
        SP = node
            ?.ResolvePath("sp")
            ?.Children.Select(p => (IQuestTemplateActSP)new QuestTemplateActSP(p.Cache()))
            .ToFrozenSet();
    }
