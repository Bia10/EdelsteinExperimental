...
{
    public static bool TryAddPercent(
        ref IReadOnlyList<IRateModifier> result,
        string source,
        int? value
    )
    {
        ...
        result = result.Count == 0 ? [modifier] : [.. result, modifier];
        return true;
    }

    public static IReadOnlyList<IRateModifier> Single(
        string source,
        double multiplier,
        int? priority = null
