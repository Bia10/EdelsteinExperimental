...
public sealed class TemporaryStatRateModifierSource : IRateModifierSource
{
    ...
    )
    {
        ...
        RateModifierBuilder.TryAddPercent(
            ref result,
            "event-rate",
            stats[TemporaryStatType.EventRate]?.Value
        );
        ...
        switch (type)
        {
            case RateType.Exp:
                RateModifierBuilder.TryAddPercent(
                    ref result,
                    "exp-buff",
                    stats[TemporaryStatType.ExpBuffRate]?.Value
                );
                if (stats[TemporaryStatType.HolySymbol] != null)
                    RateModifierBuilder.TryAddPercent(
                        ref result,
                        "holySymbol-exp",
                        stats[TemporaryStatType.HolySymbol]?.Value
                    );
            ...
            case RateType.Meso:
                RateModifierBuilder.TryAddPercent(
                    ref result,
                    "meso-up",
                    stats[TemporaryStatType.MesoUp]?.Value
                );
                RateModifierBuilder.TryAddPercent(
                    ref result,
                    "meso-up-by-item",
                    stats[TemporaryStatType.MesoUpByItem]?.Value
                );
                if (stats[TemporaryStatType.Dice] != null)
                    RateModifierBuilder.TryAddPercent(
                        ref result,
                        "dice-meso",
                        stats.DiceInfo.MESOr
                    );
                break;
            case RateType.Drop:
                RateModifierBuilder.TryAddPercent(
                    ref result,
                    "item-up-by-item",
                    stats[TemporaryStatType.ItemUpByItem]?.Value
                );
                break;
        }
