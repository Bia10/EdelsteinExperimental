...
public class FieldUserModify : IFieldUserModify
{
    ...
    public async Task Stats(IModifyStatContext context, bool exclRequest = false)
    {
        ...
        if (
            _user.StageUser.Party != null
            && (
                context.Flag.HasFlag(ModifyStatType.Level)
                || context.Flag.HasFlag(ModifyStatType.Job)
            )
        )
            _ = _user.StageUser.Context.Services.Party.UpdateLevelOrJob(
                new PartyUpdateLevelOrJobRequest(
    ...
    public async Task StatsForced(IModifyStatForcedContext context)
    {
        ...
        if (context.Flag > 0)
        {
            ...
            if ((context.Flag & ModifyStatForcedType.STR) != 0)
                setPacket.WriteShort(context.STR);
            if ((context.Flag & ModifyStatForcedType.DEX) != 0)
                setPacket.WriteShort(context.DEX);
            if ((context.Flag & ModifyStatForcedType.INT) != 0)
                setPacket.WriteShort(context.INT);
            if ((context.Flag & ModifyStatForcedType.LUK) != 0)
                setPacket.WriteShort(context.LUK);

            if ((context.Flag & ModifyStatForcedType.PAD) != 0)
                setPacket.WriteShort(context.PAD);
            if ((context.Flag & ModifyStatForcedType.PDD) != 0)
                setPacket.WriteShort(context.PDD);
            if ((context.Flag & ModifyStatForcedType.MAD) != 0)
                setPacket.WriteShort(context.MAD);
            if ((context.Flag & ModifyStatForcedType.MDD) != 0)
                setPacket.WriteShort(context.MDD);
            if ((context.Flag & ModifyStatForcedType.EVA) != 0)
                setPacket.WriteShort(context.EVA);
            if ((context.Flag & ModifyStatForcedType.ACC) != 0)
                setPacket.WriteShort(context.ACC);

            if ((context.Flag & ModifyStatForcedType.Speed) != 0)
                setPacket.WriteByte(context.Speed);
            if ((context.Flag & ModifyStatForcedType.Jump) != 0)
                setPacket.WriteByte(context.Jump);

            if ((context.Flag & ModifyStatForcedType.SpeedMax) != 0)
                setPacket.WriteByte(context.SpeedMax);
    ...
    )
    {
        var context = new ModifyInventoryGroupContext(
            _user.Character.Inventories,
            _user.StageUser.Context.Templates.Item
        );
        action?.Invoke(context);
    ...
    {
        var isUpdateReset =
            context.HistoryReset.Records.Any() || context.HistoryReset.HasTwoStateStats();
        var isUpdateSet = context.HistorySet.Records.Any() || context.HistorySet.HasTwoStateStats();
        if (!IsRequireUpdate)
        ...
        if (isUpdateReset)
        {
            ...
            using var resetRemotePacket = new PacketWriter(
                PacketSendOperations.UserTemporaryStatReset
            );
