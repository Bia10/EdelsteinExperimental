...
public class MobRewardPoolTemplateLoader : ITemplateLoader
{
    ...
    public MobRewardPoolTemplateLoader(
        IDataNamespace data,
        ITemplateManager<MobRewardPoolTemplate> manager
    )
    {
        _data = data;
        _manager = manager;
    ...
    public async Task<int> Load()
    {
        await Task.WhenAll(
            _data
                .ResolvePath("Server/Reward.img")
                ?.Children.Select(async n =>
                {
                    var id = Convert.ToInt32(n.Name);
                    await _manager.Insert(
                        new TemplateProviderEager<MobRewardPoolTemplate>(
                            id,
