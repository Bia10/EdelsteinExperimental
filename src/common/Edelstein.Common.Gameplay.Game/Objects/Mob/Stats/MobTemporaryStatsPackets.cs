...
public static class MobTemporaryStatsPackets
{
    ...
    public static void WriteMobTemporaryStatsFlag(
        this IPacketWriter writer,
        IMobTemporaryStats mobStats
    )
    {
    ...
    public static void WriteMobTemporaryStats(
        this IPacketWriter writer,
        IMobTemporaryStats mobStats
    )
    {
        ...
        if (mobStats.BurnedInfo.Count > 0)
        {
            ...
            foreach (var burned in mobStats.BurnedInfo)
            {
                ...
                writer.WriteInt(
                    (int)(
                        (burned.DateExpire - now).TotalMilliseconds
                        / burned.Interval.TotalMilliseconds
                    )
                );
            }
        }
        ...
        if (
            stats.ContainsKey(MobTemporaryStatType.PCounter)
            || stats.ContainsKey(MobTemporaryStatType.MCounter)
        )
