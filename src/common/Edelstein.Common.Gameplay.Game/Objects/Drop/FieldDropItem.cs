...
public class FieldDropItem : AbstractFieldDrop
{
    ...
    public FieldDropItem(
        ...
        : base(position, ownType, ownerID, sourceID) => _item = item;
    ...
    protected override Task<bool> Check(IFieldUser user) =>
        Task.FromResult(
            user.StageUser.Context.Managers.Inventory.HasSlotFor(user.Character.Inventories, _item)
        );
    ...
    protected override async Task Update(IFieldUser user)
    {
        ...
        await user.Message(
            new DropPickUpItemMessage(_item.ID, _item is IItemSlotBundle bundle ? bundle.Number : 1)
        );
    }
}
