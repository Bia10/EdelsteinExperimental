...
{
    public GameStageUser(ISocket socket, GameContext context)
        : base(socket)
    {
        Context = context;
    }
    ...
    public int CrcKey { get; set; } = Random.Shared.Next();

    public override Task Migrate(string serverID, IPacket? packet = null) =>
        Context.Pipelines.UserMigrate.Process(
            new UserMigrate<IGameStageUser>(this, serverID, packet)
        );
    ...
    public override Task OnException(Exception exception) =>
        Context.Pipelines.UserOnException.Process(
            new UserOnException<IGameStageUser>(this, exception)
        );
