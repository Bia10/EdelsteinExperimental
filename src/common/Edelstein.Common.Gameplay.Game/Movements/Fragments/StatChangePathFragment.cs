    ...
    where TMoveAction : IMoveAction
{
    ...
    public StatChangePathFragment(MovePathFragmentType type)
        : base(type) { }

    protected override void ReadBody(IPacketReader reader) => _stat = reader.ReadBool();

    protected override void WriteBody(IPacketWriter writer) => writer.WriteBool(_stat);
