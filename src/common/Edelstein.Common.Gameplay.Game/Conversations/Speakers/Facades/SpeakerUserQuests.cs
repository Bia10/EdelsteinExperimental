...
public class SpeakerUserQuests : ISpeakerUserQuests
{
    ...
    private IDictionary<string, string> GetDictionaryFromRecord(string record) =>
        record
            ...
            .Select(s => s.Split("="))
            .Select(s =>
                Tuple.Create(s.Length > 0 ? s[0] : string.Empty, s.Length > 1 ? s[1] : string.Empty)
            )
    ...
    public string GetRecord(short questID, string key) =>
        GetDictionaryFromRecord(GetRecord(questID)).TryGetValue(key, out var value)
            ? value
            : string.Empty;
    ...
    public string GetRecordEx(short questID, string key) =>
        GetDictionaryFromRecord(GetRecordEx(questID)).TryGetValue(key, out var value)
            ? value
            : string.Empty;

    public int GetStatus(short questID) =>
        _user.Character.QuestCompletes.Records.ContainsKey(questID) ? 2
        : _user.Character.QuestRecords.Records.ContainsKey(questID) ? 1
        : 0;
