...
{
    public static Task DispatchMobCrcKeyChanged(this IGameStageUser user, int objectID = 0)
    {
        using var packet = new PacketWriter(PacketSendOperations.MobCrcKeyChanged);
        packet.WriteInt(objectID);
        packet.WriteInt(user.CrcKey);
