...
public class DamageCalculator : IDamageCalculator
{
    ...
    public void Skip() => _rndGenForCharacter.Next(new uint[RndSize]);
    ...
    )
    {
        ...
        switch (attack.SkillID)
        {
            case Skill.SniperStrafe:
                var ultimateStrafeSkill = await _skills.Retrieve(
                    Skill.CrossbowmasterUltimateStrafe
                );
                var ultimateStrafeLevel = ultimateStrafeSkill?[
                    stats.SkillLevels[Skill.CrossbowmasterUltimateStrafe]
                ];
        ...
        foreach (var kv in character.Skills.Records)
        {
            ...
            if (skillTemplate == null || levelTemplate == null)
                continue;
            if (!skillTemplate.IsPSD)
                continue;
            if (!skillTemplate.PsdSkill.Contains(attack.SkillID))
                continue;
        ...
        for (var i = 0; i < result.Length; i++)
        {
            ...
            if (
                attack.SkillID
                is Skill.HeroRush
                    or Skill.PaladinRush
                    or Skill.DarkknightRush
                    or Skill.Dual3HustleRush
                    or Skill.PaladinBlast
                    or Skill.Dual4FlyingAssaulter
                    or Skill.Dual2SlashStorm
                    or Skill.Dual4BloodyStorm
            )
                random.Skip();

            if (i >= damagePerMob)
            {
                var shadowPartnerSkillID = character
                    .TemporaryStats[TemporaryStatType.ShadowPartner]
                    ?.Reason;
                var shadowPartnerSkill = await _skills.Retrieve(shadowPartnerSkillID ?? 0);
                var shadowPartnerLevel = shadowPartnerSkill?[
                    stats.SkillLevels[shadowPartnerSkillID ?? 0]
                ];

                result[i] = new Damage(
                    damageCalcShadowPartner[i - damagePerMob] * (shadowPartnerLevel?.X ?? 0) / 100
                );
                continue;
            }
            ...
            switch (attack.SkillID)
            {
                case Skill.RogueLuckySeven or Skill.NightwalkerLuckySeven:
                    ...
                    damage =
                        GetRandomInRange(random.Next(), stats.LUK, stats.LUK * 1.4)
                        * 5.5
                        * stats.PAD
                        / 100d;
                    break;
            ...
            if (
                !SkillConstants.IsShootAction(attack.AttackAction)
            ...
            )
            {
                switch (weaponType)
                {
                    ...
                    case WeaponType.ThrowingGlove
                        when attack.SkillID != Skill.NightlordNinjaStorm
            ...
            if (!isLuckySevenOrTripleThrow)
                damage = GetDamageAdjustedByRandom(
                    damage,
                    stats.Mastery,
                    ItemConstants.GetMasteryConstByWeaponType(weaponType),
                    random.Next()
                );
            ...
            // ElemBoost
            var damageAdjustedByElemAttr =
                skill != null
                    ? GetDamageAdjustedByElemAttr(
                        damage,
                        mobStats.ElementAttributes[skill.Element],
                        1.0,
                        0.0
                    )
                    : damage;
            ...
            if (weaponChargeStat != null)
            {
                ...
                var weaponChargeLevel = weaponChargeSkill?[
                    stats.SkillLevels[weaponChargeStat.Reason]
                ];
                ...
                if (weaponChargeLevel != null)
                {
                    ...
                    var damageAdjustedByChargedElemAttr = GetDamageAdjustedByElemAttr(
                        amp * damage,
                        mobStats.ElementAttributes[
                            SkillConstants.GetElementByChargedSkill(weaponChargeStat.Reason)
                        ],
                        adjust,
                        0.0
                    );
                    ...
                    damage =
                        damageAdjustedByChargedElemAttr + damageAdjustedByAssistChargedElemAttr;
                }
            }

            if (
                attack.SkillID != Skill.DragonknightSacrifice
            ...
            switch (attack.SkillID)
            {
                ...
                case Skill.WarriorSlashBlast:
                {
                    ...
                    )
                    {
                        ...
                        var warriorImproveBasicLevel = warriorImproveBasicSkill?[
                            stats.SkillLevels[skillID]
                        ];

                        if (warriorImproveBasicLevel == null)
                            break;

                        skillDamageR +=
                            attack.SkillID == Skill.WarriorPowerStrike
                ...
                case Skill.KnightChargeBlow:
                {
                    ...
                    var advancedChargeLevel = advancedChargeSkill?[
                        stats.SkillLevels[Skill.PaladinAdvancedCharge]
                    ];
                ...
                case Skill.ArcherDoubleShot:
                {
                    ...
                    )
                    {
                        ...
                        var archerImproveBasicLevel = archerImproveBasicSkill?[
                            stats.SkillLevels[skillID]
                        ];

                        if (archerImproveBasicLevel == null)
                            break;

                        skillDamageR +=
                            attack.SkillID == Skill.ArcherArrowBlow
                ...
                case Skill.WildhunterFlashRain:
                    if (i == damagePerMob - 1)
                    {
                        ...
                        var flashRainLevel = flashRainSkill?[
                            stats.SkillLevels[Skill.WildhunterFlashRain]
                        ];
            ...
            if (comboCounterStat != null)
            {
                var comboCounterSkillID =
                    JobConstants.GetJobRace(character.Job) == 0
                ...
                var advComboCounterLevel = advComboCounterSkill?[
                    stats.SkillLevels[advComboCounterSkillID]
                ];

                var damagePerCombo =
                    (advComboCounterLevel?.DIPr ?? 0) + (comboCounterLevel?.DIPr ?? 0);
            ...
            if (
                mob.TemporaryStats[MobTemporaryStatType.Stun] != null
            ...
            )
            {
                ...
                var chanceAttackLevel = chanceAttackSkill?[
                    stats.SkillLevels[Skill.CrusaderChanceAttack]
                ];
            ...
            if (attack.Keydown > 0)
                damage *=
                    (90 * attack.Keydown / SkillConstants.GetMaxGaugeTime(attack.SkillID) + 10)
                    / 100d;
    ...
    )
    {
        ...
        for (var i = 0; i < damagePerMob; i++)
        {
            ...
            // ElemBoost
            var elementalResetStat =
                character.TemporaryStats[TemporaryStatType.ElementalReset]?.Value ?? 0;
            var damageAdjustedByElemAttr =
                skill != null
                    ? GetDamageAdjustedByElemAttr(
                        damage,
                        mobStats.ElementAttributes[skill.Element],
                        1d - elementalResetStat / 100d,
                        0d
                    )
                    : damage;
            ...
            switch (attack.SkillID)
            {
                case Skill.Mage1MagicComposition:
                    damageAdjustedByElemAttr =
                        GetDamageAdjustedByElemAttr(
                            damage * 0.5,
                            mobStats.ElementAttributes[Element.Poison],
                            1.0,
                            0d
                        )
                        + GetDamageAdjustedByElemAttr(
                            damage * 0.5,
                            mobStats.ElementAttributes[Element.Fire],
                            1.0,
                            0d
                        );
                    break;
                case Skill.Mage2MagicComposition:
                    damageAdjustedByElemAttr =
                        GetDamageAdjustedByElemAttr(
                            damage * 0.5,
                            mobStats.ElementAttributes[Element.Light],
                            1.0,
                            0d
                        )
                        + GetDamageAdjustedByElemAttr(
                            damage * 0.5,
                            mobStats.ElementAttributes[Element.Ice],
                            1.0,
                            0d
                        );
                    break;
    ...
    )
    {
        ...
        if (
            attack.SkillID
                is 0
                    or Skill.LegendFinalBlow
                    or Skill.AranDoubleSwing
                    or Skill.AranFinalToss
                    or Skill.AranFinalBlow
                    or Skill.AranOverSwingDs
                    or Skill.AranOverSwingTs
                    or Skill.AranRollingSpin
                    or Skill.AranFullSwingDs
                    or Skill.AranFullSwingTs
            && mobCount > 1
            && (
        ...
        )
        {
            for (var i = 0; i < damage.Length; i++)
                result[i] = (int)(
                    ((stats.Level / 10.0 + 20.0) * (mobCount - 1) / 100.0 + 1.0)
                    / mobCount
                    * result[i]
                );
        }
        ...
        if (
            attack.AttackActionType == AttackActionType.DualDagger
            && (attack.IsShadowPartner ? damage.Length / 2 : damage.Length) > 1
            ...
            && (
                character
                    .Inventories[ItemInventoryType.Equip]
                    ?.Items.TryGetValue(-(short)BodyPart.Weapon, out var itemDagger) ?? false
            )
            && (
                character
                    .Inventories[ItemInventoryType.Equip]
                    ?.Items.TryGetValue(-(short)BodyPart.Shield, out var itemDual) ?? false
            )
        ...
        )
        {
            ...
            for (var i = 0; i < damage.Length; i++)
                result[i] =
                    i % 2 == 0 ? (int)(result[i] * rateDagger) : (int)(result[i] * rateDual);
        }
    ...
    private static double GetDamageAdjustedByRandom(
        double damage,
        int mastery,
        double masteryConst,
        uint rand
    ...
    private static double GetDamageAdjustedByElemAttr(
        double damage,
