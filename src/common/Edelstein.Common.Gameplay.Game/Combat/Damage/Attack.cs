...
public class Attack : IAttack, IPacketReadable
{
    ...
    {
        FieldKey = reader.ReadByte(); // bCurFieldKey
        Dr0 = reader.ReadInt(); // dr0
        Dr1 = reader.ReadInt(); // dr1

        var v6 = reader.ReadByte();
        DamagePerMob = (byte)(v6 >> 0 & 0xF);
        MobCount = (byte)(v6 >> 4 & 0xF);

        Dr2 = reader.ReadInt(); // dr2
        Dr3 = reader.ReadInt(); // dr3

        SkillID = reader.ReadInt();
        IsCombatOrders = reader.ReadBool();

        DrRand = reader.ReadInt(); // dr rand
        Crc = reader.ReadInt(); // crc
