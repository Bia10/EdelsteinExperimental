    ...
    public override async Task HandleSkillUse(ISkillContext context, IFieldUser user)
    {
        switch (context.Skill?.ID)
        {
            case Skill.BmageAuraBlue:
            {
                ...
                {
                    auraSkill = await user.StageUser.Context.Templates.Skill.Retrieve(
                        Skill.BmageAuraBlueAdvanced
                    );
                ...
                if (auraSkill != null && auraLevel != null)
                {
                    ...
                    await user.ModifyTemporaryStats(s =>
                        s.Set(TemporaryStatType.Aura, context.SkillLevel!.Level, context.Skill!.ID)
                    );
                }
