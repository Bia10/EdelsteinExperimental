...
public class ShopOnPacketUserTransferFieldRequestPlug
    : IPipelinePlug<TradeOnPacketUserTransferFieldRequest>
{
    ...
    {
        if (message.User.FromServerID == null)
            return;

        var response = await _serverService.GetByID(
            new ServerGetByIDRequest(message.User.FromServerID)
        );
