...
public abstract class AbstractUserOnPacketMigrateInPlug<TStage, TStageUser>
    : IPipelinePlug<UserOnPacketMigrateIn<TStageUser>>
    ...
    where TStageUser : class, IStageUser<TStageUser>
{
    ...
    public async Task Handle(IPipelineContext ctx, UserOnPacketMigrateIn<TStageUser> message)
    {
        ...
        var migrationResponse = await _migrationService.Claim(
            new MigrationClaimRequest(message.CharacterID, _stage.ID, message.Key)
        );

        if (
            migrationResponse.Result != MigrationResult.Success
            || migrationResponse.Migration == null
        )
        {
            await message.User.Disconnect();
            _logger.LogDebug("Failed to migrate in for character {ID}", message.CharacterID);
            return;
        }

        var sessionServerResponse = await _sessionService.UpdateServer(
            new SessionUpdateServerRequest(migrationResponse.Migration.Account.ID, _stage.ID)
        );
        var sessionCharacterResponse = await _sessionService.UpdateCharacter(
            new SessionUpdateCharacterRequest(
        ...
        {
            await message.User.Disconnect();
            _logger.LogDebug("Failed to update session for character {ID}", message.CharacterID);
            return;
        }
        ...
        message.User.Friends = (
            await _friendService.Load(new FriendLoadRequest(message.User.Character.ID))
        ).Friends;
        message.User.Party = (
            await _partyService.Load(new PartyLoadRequest(message.User.Character.ID))
        ).PartyMembership;
        message.User.Guild = (
            await _guildService.Load(new GuildLoadRequest(message.User.Character.ID))
        ).GuildMembership;
