...
public class ItemOptionTemplateLoader : ITemplateLoader
{
    ...
    public ItemOptionTemplateLoader(
        IDataNamespace data,
        ITemplateManager<IItemOptionTemplate> manager
    )
    {
        _data = data;
        _manager = manager;
    ...
    public async Task<int> Load()
    {
        await Task.WhenAll(
            _data
                .ResolvePath("Item/ItemOption.img")
                ?.Children.Select(async n =>
                {
                    var id = Convert.ToInt32(n.Name);
                    await _manager.Insert(
                        new TemplateProviderEager<IItemOptionTemplate>(
                            id,
                            new ItemOptionTemplate(id, n.Cache())
                        )
                    );
                })
                ?? Array.Empty<Task>()
