...
public class ItemStringTemplateLoader : ITemplateLoader
{
    ...
    public ItemStringTemplateLoader(
        IDataNamespace data,
        ITemplateManager<IItemStringTemplate> manager
    )
    {
        _data = data;
        _manager = manager;
    ...
    public async Task<int> Load()
    {
            ...
            _data
                ...
                .Select(async n =>
                {
                    ...
                    await _manager.Insert(
                        new TemplateProviderEager<IItemStringTemplate>(
                            id,
                            new ItemStringTemplate(id, n.Cache())
                        )
                    );
                })
                ?? Array.Empty<Task>()
        ...
        foreach (var sub in new[] { "Consume", "Ins", "Cash" })
            await Task.WhenAll(
                _data
                    .ResolvePath($"String/{sub}.img")
                    ?.Children.Where(c => c.Name.All(char.IsDigit))
                    .Select(async n =>
                    {
                        var id = Convert.ToInt32(n.Name);
                        await _manager.Insert(
